...
调度时间（Asia/Shanghai，day_of_week=1-5 工作日）:
...
async def _run_for_markets(markets: Iterable[str], label: str) -> dict:
    ...
    不自己做限流：queue_service 已经限制每用户 3 并发 / 全局 10 并发。
    跨用户串行遍历（await create_analysis_task），单用户内用 asyncio.create_task
    让 execute_analysis_background 真正并发进 queue。
    """
        ...
        async for doc in db.user_favorites.find({}):
            ...
            for fav in user_favs:
                ...
                try:
                    ...
                    # 真正的分析走后台（queue_service 会做 per-user 3 并发限流）
                    asyncio.create_task(
                        simple_service.execute_analysis_background(task_id, user_id, req),
